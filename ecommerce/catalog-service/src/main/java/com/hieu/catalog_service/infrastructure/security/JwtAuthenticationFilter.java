package com.hieu.catalog_service.infrastructure.security;
...
/**
 * Stateless JWT authentication filter for catalog-service.
 *
 * <p>catalog-service doesn't own users — we only verify signatures and trust the claims
 * the gateway / auth-service have issued. Bearer token or {@code ACCESS_TOKEN} cookie
 * both work; invalid / missing tokens leave the SecurityContext empty so public
 * endpoints keep responding.
 *
 * <p>Authorities are built from both {@code roles} and {@code permissions} claims so
 * controllers can use either {@code @PreAuthorize("hasRole('ADMIN')")} or
 * {@code @PreAuthorize("hasAuthority('catalog:write')")}.
 */
@Component
